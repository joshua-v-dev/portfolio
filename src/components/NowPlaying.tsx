      ...
      <div className="flex flex-row items-center space-x-2">
        {(await playing) ? (
          ...
          <svg className="ml-auto mt-[-2px] h-4 w-4" viewBox="0 0 168 168">
            <path
              fill="#1ED760"
              d="M83.996.277C37.747.277.253 37.77.253 84.019c0 46.251 37.494 83.741 83.743 83.741 46.254 0 83.744-37.49 83.744-83.741 0-46.246-37.49-83.738-83.745-83.738l.001-.004zm38.404 120.78a5.217 5.217 0 01-7.18 1.73c-19.662-12.01-44.414-14.73-73.564-8.07a5.222 5.222 0 01-3.59-9.5c29.15-6.66 53.902-3.94 73.564 8.07a5.217 5.217 0 011.73 7.18z"
            />
            <path
              fill="#1ED760"
              d="M122.4 97.497a5.217 5.217 0 01-7.18 1.73c-22.074-13.49-56.387-17.5-83.448-9.764a5.222 5.222 0 01-3.59-9.5c31.074-8.08 65.387-4.07 94.072 11.127a5.217 5.217 0 011.73 7.18z"
            />
          </svg>
        )}
      </div>
      ...
      <div className="inline-flex w-full max-w-full flex-col truncate sm:flex-row">
        {(await props.data)?.songUrl ? (
          <a
            ...
            href={(await props.data).songUrl}
            target="_blank"
            rel="noopener noreferrer"
          >
            {(await props.data).title}
          </a>
        ) : (
        ...
        <p className="capsize max-w-max truncate text-gray-500 dark:text-gray-300">
          {(await props.data)?.artist ?? "Spotify"}
        </p>
      </div>
    </div>
  );
